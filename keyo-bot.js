...
async function _buildSystemPrompt(unidadeId, sessao) {
  ...
  const unidade = db.unidades.find(u => String(u.id) === String(unidadeId))
    || null; // [FIX] Sem fallback silencioso — unidadeId inválido não injeta dados de outra unidade no prompt
  ...
  // [v2-AGENDAMENTO] Toggle do ERP: unidade que NÃO faz agendamento online.
  const semAgendamento = unidade?.aceitaAgendamento === false;
  ...
  const ehFer = db.feriados.some(f => f.data === dataHojeISO);

  // [FIX-PRECO] Preço vem da SALA, não da unidade.
  // Helper: calcula preço da sala para hoje (retorna null se não configurado).
  function _precoSalaHoje(s) {
    if (ehFer) return s.precoFeriado ?? s.preco_feriado ?? null;
    if (ehFds) return s.precoFimSemana ?? s.preco_fim_semana ?? null;
    return s.precoSemana ?? s.preco_semana ?? null;
  }

  // [FIX-SALAS] Garante comparação de string para evitar mismatch número vs string
  // [FIX] Guard: unidade fechada (emObra ou inativa) não expõe salas no prompt
  const _unidadeFechada = unidade?.emObra === true || unidade?.ativa === false;
  const salasDesc = _unidadeFechada
    ? '⚠️ Esta unidade está temporariamente fechada — NÃO liste salas nem ofereça reservas.'
    : salas.length
      ? salas.map(s => {
          const preco = _precoSalaHoje(s);
          const precoTexto = preco != null
            ? `R$ ${preco} p/pessoa`
            : 'preço a consultar com a equipe';
  ...
  function _blocoUnidade(u) {
   ...
   🔧 Esta unidade está ${motivoFechamento} e não está atendendo no momento.${previsaoAbertura}
   ❌ NÃO ofereça reservas, horários ou salas desta unidade.
   ✅ Se o cliente perguntar por esta unidade: informe com simpatia que está ${motivoFechamento} e sugira outra unidade disponível. NUNCA invente data de reabertura se não houver previsão cadastrada.`;
    }
...
`) : '      • Consulte a equipe.';
    ...
    const salasTxt = salasU.length ? salasU.map(s => {
      const preco = _precoSalaHoje(s);
      const precoTxt = preco != null ? 'R$ ' + preco + ' p/pessoa' : 'preço a consultar';
...
`) : '      • (nenhuma sala cadastrada nesta unidade)';
    const semAg = u?.aceitaAgendamento === false;
    // [FIX-ENDERECO-SYNC] Aracaju: "Shopping Jardins" (Supabase) → "Shopping Praia Sul" (ERP)
    // Enquanto Supabase não for sincronizado, esta regra garante a resposta correta no bot
    const enderecoBotCorrigido = (String(u.id) === '1' && u.endereco === 'Shopping Jardins') ? 'Shopping Praia Sul' : u.endereco;
    const cab = '🏢 *' + u.nome + '* (id: ' + u.id + ')' + (enderecoBotCorrigido ? `
   📍 ` + enderecoBotCorrigido : '');
    const agAviso = semAg ? `
   ⚠️ NÃO faz agendamento online — é só chegar! NÃO ofereça reserva/Pix aqui; informe os horários e, se insistirem, use escalar_humano.` : '';
...
${unidadesDesc}
━━━━━━━━━━━━━━━━━━━━━━━━
${_unidadeFechada ? `🚨 ATENÇÃO — UNIDADE DO CLIENTE ESTÁ FECHADA:
- Esta unidade está temporariamente fechada e NÃO está em operação no momento.
- Informe o cliente com simpatia: "Oi! 😊 Nossa unidade está temporariamente fechada no momento, mas assim que reabrir você vai ser um dos primeiros a saber!"
- ${unidade?.previsaoAbertura ? `Previsão de reabertura: *${unidade.previsaoAbertura}*. Informe ao cliente.` : `Não há previsão de reabertura cadastrada — NUNCA invente uma data.`}
- NÃO ofereça reservas, horários, salas ou pagamento Pix.
- Se o cliente quiser mais informações ou tiver urgência: use *escalar_humano* para um atendente humano ajudar.
- Se houver outra unidade disponível, mencione com simpatia (ex: "Temos também a unidade de Salvador, caso queira!")

━━━━━━━━━━━━━━━━━━━━━━━━
` : ''}📌 REGRA DE UNIDADE (essencial):
- Descubra logo de qual unidade o cliente fala. Se não disser e houver dúvida, PERGUNTE com gentileza ("Você quer falar de qual unidade? 😊").
...
━━━━━━━━━━━━━━━━━━━━━━━━
${semAgendamento ? `🚫 ESTA UNIDADE NÃO FAZ AGENDAMENTO ONLINE — LEIA COM ATENÇÃO:
- Esta unidade opera APENAS por ordem de chegada. NÃO há reserva, NÃO há Pix, NÃO há slot a consultar.
- Se o cliente perguntar sobre reserva, responda com simpatia: "Aqui não precisamos agendar! 😊 É só chegar em um dos nossos horários e a equipe te atende na ordem de chegada."
- NUNCA chame consultar_horarios, gerar_pagamento_pix ou confirmar_reserva para esta unidade.
- Se o cliente insistir em agendar ou pagar online, use escalar_humano para um atendente explicar.
- Informe os horários de funcionamento normalmente — o cliente pode (e deve) se planejar pra chegar.
- IGNORE completamente o fluxo de reserva abaixo para esta unidade.
...
- 🎂 *Aniversariante do dia*: em grupos com mais de 5 pagantes, o aniversariante não paga; em grupos com menos de 5, ganha R$ 10 de desconto seguindo o Instagram da loja e com o app do shopping. Só o aniversariante do dia, com documento oficial com foto.
...
━━━━━━━━━━━━━━━━━━━━━━━━
🏢 FLUXO CORPORATIVO (evento de empresa):
Quando for evento corporativo, NÃO escale na hora. Primeiro colete, com simpatia e UMA pergunta por vez:
1. Nome da empresa
2. Cidade
3. Quantidade aproximada de pessoas
4. Nome e melhor contato do responsável
Depois confirme num resumo curtinho ("Anotei: empresa X, cidade Y, ~Z pessoas, contato W — tá certinho? 😊") e SÓ ENTÃO chame *escalar_humano*, colocando todos esses dados no campo "motivo" (ex.: "CORPORATIVO — Empresa X, Cidade Y, ~30 pessoas, contato João 79...").

🎂 FLUXO ANIVERSÁRIO (festa):
Para festa de aniversário, colete primeiro, UMA pergunta por vez:
1. Data desejada
2. Unidade (Aracaju ou Salvador)
3. Quantidade aproximada de pessoas
4. Idade do aniversariante
Confirme num resumo curto e DEPOIS chame *escalar_humano* com tudo no "motivo" (ex.: "ANIVERSÁRIO — 15/07, Salvador, ~12 pessoas, aniversariante faz 10 anos"). Pode lembrar com carinho que pet também pode ir à festa 🐾 e da promoção do aniversariante do dia.
...
- TRIAGEM AO ESCALAR: sempre classifique a urgência com critério. CRITICO = cliente lesado agora (pagou e não recebeu, está na porta sem atendimento, ameaça exposição). ALTO = reclamação de serviço ou cancelamento de hoje/amanhã. MEDIO = alteração, orçamento corporativo, caso que exige humano sem pressa. BAIXO = sem prazo. Na dúvida entre dois níveis, escolha o mais alto. Reclamação NUNCA é BAIXO.
...
const _TOOLS = [
  ...
  {
    name: 'escalar_humano',
    description: 'Encaminha a conversa para atendente humano. Use para: cancelamentos, alterações de horário, reclamações, grupos corporativos, pagamento no local, qualquer situação fora do escopo do bot. SEMPRE classifique a urgência: CRITICO = cliente lesado ou situação que piora a cada minuto (pagou e não recebeu, está na porta sem atendimento, ameaça exposição pública); ALTO = reclamação de serviço, cancelamento de reserva de hoje/amanhã; MEDIO = alteração, orçamento corporativo, dúvida que exige humano; BAIXO = curiosidade ou pedido sem prazo.',
    input_schema: {
      type: 'object',
      properties: {
        motivo:    { type: 'string', description: 'Motivo do escalonamento, com os fatos essenciais' },
        urgencia:  { type: 'string', enum: ['CRITICO', 'ALTO', 'MEDIO', 'BAIXO'], description: 'Classificação de urgência da triagem' },
        categoria: { type: 'string', enum: ['reclamacao', 'cancelamento', 'alteracao', 'corporativo', 'pagamento', 'duvida', 'outro'], description: 'Tipo da situação' },
        urgente:   { type: 'boolean', description: '(legado) true se urgência for CRITICO ou ALTO' }
      },
...
async function _executarFerramenta(nome, input, sessao) {
  ...
  if (nome === 'consultar_horarios') {
    try {
      ...
      // [v2-AGENDAMENTO] Trava: unidade sem agendamento online não consulta slots.
      if (unidade.aceitaAgendamento === false) {
        return JSON.stringify({
          bloqueado: true,
  ...
  if (nome === 'gerar_pagamento_pix') {
    try {
      ...
      // [v2-AGENDAMENTO] Trava: nunca gera pagamento para unidade sem agendamento online.
      if (unidade.aceitaAgendamento === false) {
        return JSON.stringify({
          bloqueado: true,
  ...
  if (nome === 'escalar_humano') {
    ...
    // [v3-TRIAGEM] Classificação do Crítico: urgência vinda do próprio LLM da conversa.
    // Compatibilidade: mantém 'urgente' (boolean) para o que já lê esse campo.
    const urgencia = ['CRITICO', 'ALTO', 'MEDIO', 'BAIXO'].includes(input.urgencia)
      ? input.urgencia
      : (input.urgente ? 'ALTO' : 'MEDIO');
    const ehUrgente = input.urgente === true || urgencia === 'CRITICO' || urgencia === 'ALTO';

    // Salva na memória KEYO — um trigger no Supabase transforma este registro em
    // item de triagem do agente Crítico na keyo_brain_queue (painel/KEYO enxergam).
    await supabase.salvarMemoria('whatsapp_escalonamento', sessao.telefone, {
      ...
      urgente: ehUrgente,
      nomeCliente: sessao.cliente?.nome || sessao.nomeProvisorio || null,
    ...
    await supabase.registrarAuditoria(
      ...
      `Escalonado para humano [${urgencia}]: ${sessao.telefone} — ${input.motivo}`,
      'KEYO-BOT'
    );

    // [PEÇA-8] Notifica o atendente humano via WhatsApp — com a triagem visível
    const urgEmoji = { CRITICO: '🚨', ALTO: '⚠️', MEDIO: '📋', BAIXO: '💬' }[urgencia] || '📋';
    const urgTag = urgencia === 'CRITICO' ? '🚨 *CRÍTICO* — ' : (urgencia === 'ALTO' ? '⚠️ *URGENTE* — ' : '');
    const nomeCliente = sessao.cliente?.nome || sessao.nomeProvisorio || 'Cliente';
    const msgAtendente =
      `${urgTag}⚠️ *KEYO — Escalonamento para atendente*\n\n` +
      `👤 Cliente: *${nomeCliente}*\n` +
      `📱 Telefone: ${sessao.telefone}\n` +
      `${urgEmoji} Urgência: *${urgencia}*${input.categoria ? ` · ${input.categoria}` : ''}\n` +
      `📋 Motivo: ${input.motivo}\n` +
      `🕐 ${new Date().toLocaleString('pt-BR', { timeZone: 'America/Maceio' })}\n\n` +
      `_Acesse o ERP para ver o histórico completo._`;
    await enviarMensagem(CFG.telAtendente, msgAtendente).catch(e =>
      console.warn('[KEYO] Falha ao notificar atendente:', e.message)
    );
    ...
    setTimeout(() => {
      if (_sessoes[sessao.telefone]?.aguardaHumano) {
        ...
        _solicitarAvaliacao(sessao.telefone, _sessoes[sessao.telefone]).catch(() => {});
      }
    }, 30 * 60 * 1000);
...
async function _pensarEResponder(mensagemUsuario, sessao) {
  ...
  for (let rodada = 0; rodada < CFG.maxTurnosAgente; rodada++) {
    const payload = {
      ...
      // [CACHE v2] system em formato de bloco com cache_control: cacheia o prefixo
      // (tools + system, ~9k tokens estáveis). Leituras de cache custam 10% do preço de
      // entrada. O cache vive 5 min e é renovado a cada acerto — cobre o loop de agente
      // e mensagens seguidas do mesmo cliente. Reduz ~60-70% o custo de entrada.
    ...
    // [CACHE v2] Observabilidade: mostra acertos/escritas de cache nos logs do Railway.
    const _u = data.usage || {};
    if (_u.cache_read_input_tokens || _u.cache_creation_input_tokens) {
      console.info(`[KEYO] 💾 cache: leu ${_u.cache_read_input_tokens||0} | gravou ${_u.cache_creation_input_tokens||0} | entrada nova ${_u.input_tokens||0} | saída ${_u.output_tokens||0}`);
    }
...
async function processarPagamento(payload) {
  try {
    ...
    if (sessao.preReserva.expiraEm < Date.now()) {
      ...
      // [PEÇA-6] Libera horário e notifica atendente sobre desistência
      ...
      await enviarMensagem(CFG.telAtendente, msgAtendente).catch(() => {});
      delete sessao.preReserva;
      return;
    }
...
async function enviarConfirmacaoReserva(venda, sala, unidade) {
  ...
  // [FIX-ENDERECO-SYNC] Aplica mesma correção Aracaju que em _buildSystemPrompt
  const enderecoBotCorrigido = (String(unObj?.id) === '1' && unObj?.endereco === 'Shopping Jardins') ? 'Shopping Praia Sul' : unObj?.endereco;
...
function _agendarLembretes(venda, sala, unidade) {
  ...
  if (delay2h > 0) {
    setTimeout(async () => {
      // [FIX-ENDERECO-SYNC] Aplicar mesma correção
      const enderecoBotCorrigido2h = (String(unidade?.id) === '1' && unidade?.endereco === 'Shopping Jardins') ? 'Shopping Praia Sul' : unidade?.endereco;
      const msg =
        `⏰ *Sua aventura começa em 2 horas!*\n\n` +
        `${venda.nomeCliente || venda.nomeReserva}, prepare-se!\n` +
        `📍 ${unidade?.nome || 'EXIT Games'} — ${enderecoBotCorrigido2h || ''}\n` +
  ...
  if (_sessoes[tel]) {
    ...
    setTimeout(() => {
      ...
      if (s && !s.aguardaHumano && !s.avaliacaoChatSolicitada) {
        _solicitarAvaliacao(tel, s).catch(() => {});
      }
...
async function _enviarPesquisaPosJogo() {
  ...
  for (const tel in contatosUnicos) {
    if (contatosUnicos.hasOwnProperty(tel)) {
      ...
      try {
        await enviarMensagem(tel, msg, contato.unidadeId);
        enviados++;
        console.log(`[Brain Loop] Pesquisa enviada para ${tel}`);
      } catch (e) {
        console.warn(`[Brain Loop] Erro ao enviar para ${tel}:`, e.message);
      }
    }
  }
