...
function _montarContextoAgente(agente) {
  try {
    ...
    // [FIX-ESCOPO-UNIDADE] uniId calculado ANTES de tudo, via pdvUID() — a mesma
    // função que o resto do ERP usa pra saber a unidade ativa (trata ADM com
    // unidade selecionada via window._pdvUnidade). Antes, vendas/clientes não
    // eram filtrados por unidade nenhuma (somavam Aracaju + Salvador juntos),
    // enquanto contasPagar/contasReceber já eram — número inconsistente com o
    // rótulo "unidade ativa".
    const uniId = String(
    ...
    const clientes = clientesTodos; // clientes não têm unidadeId no cadastro atual — mantém geral
